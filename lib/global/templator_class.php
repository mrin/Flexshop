...
//--- Домашня дирректория и шаблон по умолчанию---------------------------------
var $homedir;
var $deftpl;
...
* @access public
*/
function __construct($homedir, $deftpl) {
...
//************************************************************************
// Создание переменных mini_templator из массива $m (мультиязычность)
function create_all_vars($m) {
	$keys = array_keys($m);
	foreach($keys as $key_value)
		$this->setVariable ($key_value,$m[$key_value],true);
	$this->setVariable("SHORT_LINK", "$m[URLSITE]"."/templates/".$m[TEMPLDEF]."/shop/",true);
	$this->setVariable("CURLINK", $HTTP_SERVER_VARS['PHP_SELF'],true);
}
...
* @access public
*/
function readTemplateFromFile ($fileName, $subfileName="/shop/index.tpl") {
   if (!$this->readFileIntoString($fileName,$s,$subfileName)) {
...
* @access private
*/
function readFileIntoString ($fileName, &$s, $subfileName="/shop/index.tpl") {
   if (function_exists('version_compare') && version_compare(phpversion(),"4.3.0",">=")) {
      $s = file_get_contents($this->homedir."/templates/".$this->deftpl.$fileName);
      $s = str_ireplace("{INSERT_SUB_TEMPLATE}", "<!-- $"."Include $subfileName-->",$s);
      if ($s === false) return false;
      return true; }
   $fh = fopen($this->homedir."/templates/".$this->deftpl.$fileName,"rb");
   if ($fh === false) return false;
...
function triggerError ($msg) {
   trigger_error ("Templator error: $msg", E_USER_ERROR); }

/**
* @access private
*/
function programLogicError ($errorId) {
   die ("Templator: Program logic error $errorId.\n"); }
