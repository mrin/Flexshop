...
*/
10  => array('<table border="1"><tr><td style="color:darkred; font-size:1.2em;"><b>Graph Error:</b> 
...
25001 => array('This PHP installation is not configured with the GD library. Please recompile PHP with GD support to run Graph. (Neither function imagetypes() nor imagecreatefromstring() does exist)',0),
...
25045 => array('Font families FF_HANDWRT and FF_BOOK are no longer available due to copyright problem with these fonts. Fonts can no longer be distributed with Graph. Please download fonts from http://corefonts.sourceforge.net/',0),
25046 => array('Specified TTF font family (id=%d) is unknown or does not exist. Please note that TTF fonts are not distributed with Graph for copyright reasons. You can find the MS TTF WEB-fonts (arial, courier etc) for download at http://corefonts.sourceforge.net/',1),
...
25064 => array('Minor or major step size is 0. Check that you haven\'t got an accidental SetTextTicks(0) in your code. If this is not the case you might have stumbled upon a bug in Graph. Please report this and if possible include the data that caused the problem',0),
...
25114 => array('PHP has not enough permissions to write to the cache file "%s". Please make sure that the user running PHP has write permission for this file if you wan to use the cache system with Graph.',1),
